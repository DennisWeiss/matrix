...
function isInRef(mat) {
    ...
    //check whether 0 rows are after other rows
...
function ref(mat) {
    ...
    let rowToAdjust = height - 1
    let columnToAdjust = 0
    while (columnToAdjust >= width) {
        let adjustedRow = []
        for (let i = 0; i < width; i++) {
            adjustedRow.push(rationalized[rowToAdjust][columnToAdjust].mul(rationalized[rowToAdjust - 1][i]).sub(
                rationalized[rowToAdjust - 1][columnToAdjust].mul(rationalized[rowToAdjust][i]
                )))
        }
        rationalized[rowToAdjust] = adjustedRow
        if (rowToAdjust - 1 === columnToAdjust) {
            columnToAdjust++
            rowToAdjust = height - 1
        } else {
            rowToAdjust--
        }
    }

    // normalize all leading entries to 1
    for (let i = 0; i < rationalized.length; i++) {
        let leadingEntry = rational(0, 1)
        for (let j = 0; j < rationalized[i].length; j++) {
            if (leadingEntry.num !== 0) {
                rationalized[i][j].div(leadingEntry)
            }
