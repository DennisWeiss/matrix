...
function ref(mat) {
    ...
    while (columnToAdjust < width - 1) {
        let adjustedRow = []
        for (let i = 0; i < width; i++) {
            adjustedRow.push(rationalized[rowToAdjust][columnToAdjust].mul(rationalized[rowToAdjust - 1][i])
                .sub(rationalized[rowToAdjust - 1][columnToAdjust].mul(rationalized[rowToAdjust][i])))
        }
