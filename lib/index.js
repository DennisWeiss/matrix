...
function ref(mat) {
    ...
    while (columnToAdjust < width - 1) {
        ...
        for (let i = 0; i < width; i++) {
            adjustedRow.push(mat[rowToAdjust][columnToAdjust] * mat[rowToAdjust - 1][i] - mat[rowToAdjust - 1][columnToAdjust] * mat[rowToAdjust][i])
        }
        mat[rowToAdjust] = adjustedRow
    ...
    for (let i = 0; i < mat.length; i++) {
        ...
        for (let j = 0; j < mat[i].length; j++) {
            if (leadingEntry !== 0) {
                mat[i][j] /= leadingEntry
            } else if (mat[i][j] !== 0) {
                leadingEntry = mat[i][j]
                mat[i][j] = 1
            }
        }
    }
